...
for i, s in enumerate(part_df['participant_id']):

    sub = f'/deneb_disk/RodentTools/data/openneuro/ds001890/sub-jgrAD{s}'
    print(sub)

    if part_df[part_df['participant_id'] == sub.split("/")[-1][-4:]]['genotype'].values[0] == 'C57BL/6':
        tg_or_wt[sublist.index(sub)] = 0
    elif part_df[part_df['participant_id'] == sub.split("/")[-1][-4:]]['genotype'].values[0] == '3xTG':
        tg_or_wt[sublist.index(sub)] = 1
    else:
